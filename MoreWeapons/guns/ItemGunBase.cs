...
public class ItemGunBase : MonoBehaviour {
    private                  PhysGrabObject               physGrabObject;
    private                  ItemToggle                   itemToggle;
    public                   int                          numberOfBullets = 1;
    [Range(0f, 65f)] public  float                        gunRandomSpread;
    ...
    [Range(0f, 100f)] public float                        misfirePercentageChange = 50f;
    ...
    public void SetMaxAmmo(int maxAmmo) {
        batteryDrain = itemBattery.batteryLife / maxAmmo;
    }
    ...
    private void Update() {
        ...
        if (triggerAnimationActive) {
            ...
            gunTrigger.localRotation =
                Quaternion.Euler(num * triggerAnimationCurve.Evaluate(triggerAnimationEval), 0f, 0f);
    ...
    private void UpdateMaster() {
        ...
        if (physGrabObject.grabbed) {
            ...
            bool isItemRotating      = false;
            bool isPlayerCrouchCrawl = false;
            bool flag3               = true;

            foreach (PhysGrabber item in physGrabObject.playerGrabbing) {
                if (flag3) {
                    if (item.playerAvatar.isCrouching || item.playerAvatar.isCrawling) {
                        isPlayerCrouchCrawl = true;
                    }
    ...
    public void Misfire() {
        if (!physGrabObject.grabbed
            && !physGrabObject.hasNeverBeenGrabbed
            && SemiFunc.IsMasterClientOrSingleplayer()
            && (float)Random.Range(0, 100) < misfirePercentageChange) {
            Shoot();
        }
    }
    ...
    private void MuzzleFlash() {
        Object.Instantiate(muzzleFlashPrefab, gunMuzzle.position, gunMuzzle.rotation, gunMuzzle)
              .GetComponent<ItemGunMuzzleFlash>().ActivateAllEffects();
    }
    ...
    public void ShootRPC() {
        ...
        SemiFunc.CameraShakeImpactDistance(gunMuzzle.position, 5f * cameraShakeMultiplier, 0.1f, distanceMin,
            distanceMax);
        SemiFunc.CameraShakeDistance(gunMuzzle.position, 0.1f * cameraShakeMultiplier, 0.1f * cameraShakeMultiplier,
        ...
        physGrabObject.rb.AddForceAtPosition(-gunMuzzle.forward * gunRecoilForce, gunMuzzle.position,
            ForceMode.Impulse);
        
        /* If we're not draining a full bar, drain the amount defined */
        ...
        for (int i = 0; i < numberOfBullets; i++) {
            ...
            if (Physics.Raycast(gunMuzzle.position, vector, out var hitInfo, gunRange,
                    (int)SemiFunc.LayerMaskGetVisionObstruct() + LayerMask.GetMask("Enemy"))) {
    ...
    public void ShootBulletRPC(Vector3 _endPosition, bool _hit) {
        ...
        ItemGunBullet component = Object.Instantiate(bulletPrefab, gunMuzzle.position, gunMuzzle.rotation)
